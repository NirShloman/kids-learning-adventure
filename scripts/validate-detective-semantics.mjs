...
export function validateSemantics(item) {
  ...
  if (item.options) {
    ...
    if (item.options.length !== (rule === "compare" ? 2 : p.choices))
  ...
  for (const v of allVisuals) {
    ...
    if (v.kind === "shape" && shapeFor(v.value)?.name !== v.label) fail("shape label disagrees with contour");
    if (v.kind === "color" && !colors.some((c) => c.hex === v.value && c.name === v.label))
      fail("color label disagrees with swatch");
    if (v.kind === "text" && v.value !== v.label) fail("text differs from spoken label");
    if (v.kind === "emoji") {
      const word = soundFor(v.label)?.slice(2).find(entry=>entry[0]===v.label);
      const expectedImage = objectFor(v.label)?.emoji ?? word?.[1] ?? groups[v.label]?.[0]?.[1]
        ?? ({'חפשו את האות':'🔎','מחפשים צורה':'🔎','בוחרים צבע':'🎨'})[v.label];
      if (v.value !== expectedImage) fail("picture does not denote its spoken label");
    }
    if (v.kind === "quantity" && !['הקבוצה הראשונה','הקבוצה השנייה'].includes(v.label)) {
      if (!objects.some(object=>v.value===object.emoji&&v.label===`${v.count} ${object.name}`)) fail("quantity label disagrees with count or picture");
    }
  }
  const optionDomain = {
    'letter-recognition': v=>v.kind==='text'&&alphabet.includes(v.value),
    'initial-letter': v=>v.kind==='text'&&alphabet.includes(v.value),
    'same-sound': v=>v.kind==='emoji'&&Boolean(soundFor(v.label)),
    shape: v=>v.kind==='shape', 'find-shape': v=>v.kind==='shape', 'odd-shape': v=>v.kind==='shape',
    sides: v=>v.kind==='text'&&/^[0-9]+$/.test(v.value)&&Number(v.value)<=8,
    color: v=>v.kind==='color', 'find-color': v=>v.kind==='color', 'odd-color': v=>v.kind==='color', 'sort-color': v=>v.kind==='color',
    category: v=>v.kind==='emoji'&&Object.hasOwn(groups,v.label),
    repeat: v=>v.kind==='emoji'&&Boolean(objectFor(v.label)),
    count: v=>v.kind==='text', next: v=>v.kind==='text', add: v=>v.kind==='text', 'number-pattern': v=>v.kind==='text',
    compare: v=>v.kind==='quantity'
  }[rule];
  for(const answer of item.options ?? []) if(!answer.visualToken || !optionDomain?.(answer.visualToken)) fail('distractor is outside the task domain');
  if (rule==='next' && !equal(visuals.map(v=>v.value),[String(o[0]),'?'])) fail('displayed counting step differs from question');
  if (rule==='letter-recognition' && visuals[0]?.kind==='text' && visuals[0].value!==o[0]) fail('displayed letter differs from question');
  if (['initial-letter','same-sound'].includes(rule) && visuals[0]?.label!==o[0]) fail('displayed word differs from spoken question');
  ...
  if (
    rule === "category" &&
    (item.itemName !== o[0] || visuals[0]?.value !== objectFor(o[0])?.emoji)
  )
