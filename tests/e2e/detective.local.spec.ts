...
test('detective smoke: hint, automatic progression, checkpoint and collection',async({page},testInfo)=>{
...
test('adaptive uses the same visible activities and finishes a mixed discovery',async({page})=>{
  await bootDetective(page,5,'hard');await page.getByRole('button',{name:'🌱 המסלול שלי'}).click(); await page.getByRole('button',{name:'מתחילים תרגול מותאם'}).click();
...
for(const game of ['matching','memory'] as const)test(`${game} supports a controlled hint and a complete board`,async({page})=>{
  ...
  await expect(page.locator('.detective-pair-card.is-hint')).toHaveCount(0);
  await finishDetective(page);await expect(page.getByTestId('discovery-summary')).toBeVisible();
});
